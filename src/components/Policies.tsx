...
export default function Policies({ isDark }: { isDark: boolean }) {
  ...
  // "Sold & Approved" folds Sold/Issued + Approved together because both are policies
  // currently in force from the agency's day-to-day perspective (issued = bound + paid;
  // approved = underwriting-approved and about to bind). One merged card is what the
  // sales team actually looks at.
    ...
    <div className="flex flex-col flex-1 min-h-0" style={{ fontFamily: FONT }} onClick={closeAllDropdowns}>
      ...
      >
        ...
        {statusSummary.map((s) => {
          ...
          // single-status cards (Renewals) and multi-status cards (Sold & Approved, Action Required).
