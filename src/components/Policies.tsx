                ...
                <div className="relative" style={{ width: 220 }} onClick={e => e.stopPropagation()}>
                  ...
                  {actionOpen && (
                    <div className="absolute left-0 right-0 top-full mt-1 z-20 rounded-lg shadow-lg overflow-hidden"
                      ...
                      {["Renew Policy", "Request Endorsement", "Download Documents", "Cancel Policy"].map(a => (
        ...
        <div className="overflow-y-auto">
          {pageItems.length === 0 ? (
            ...
            <div key={p.id} className="grid px-5 py-3.5 items-center gap-4 transition-colors cursor-pointer"
              ...
              onMouseEnter={e => (e.currentTarget.style.background = c.hoverBg)}
              onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
              <div className="text-[12px]" style={{ fontFamily: FONT, color: c.text }}>{p.created}</div>
              <div className="text-[12px] font-semibold" style={{ fontFamily: FONT, color: c.linkColor }}>{p.submissionId}</div>
              <div className="text-[12px]" style={{ fontFamily: FONT, color: c.text }}>{p.applicant}</div>
              <div className="text-[12px] pl-[5px]" style={{ fontFamily: FONT, color: c.text }}>{p.dba}</div>
              <div className="text-[12px] pl-[5px]" style={{ fontFamily: FONT, color: c.text }}>{p.effective}</div>
              <div className="text-[12px] pl-[15px]" style={{ fontFamily: FONT, color: c.text }}>{p.lob}</div>
              <div className="text-[12px] pl-[25px]" style={{ fontFamily: FONT, color: c.text }}>{p.status}</div>
              <div className="text-[12px] pl-[25px]" style={{ fontFamily: FONT, color: c.text }}>{p.producer}</div>
            </div>
          ))}
        </div>
      </div>
